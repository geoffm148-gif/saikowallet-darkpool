...
const ETH_PRICE_CACHE_KEY = 'saiko:ethPriceUsd';
const TOKEN_PRICES_CACHE_PREFIX = 'saiko:tokenPrices:';
const PRICE_TTL = 60_000; // 1 minute

/** Fetch current ETH price in USD. Cached 1 minute. */
export async function fetchEthPrice(): Promise<number> {
  const cached = cacheGet<number>(ETH_PRICE_CACHE_KEY, PRICE_TTL);
  if (cached !== null) return cached;
  try {
    const res = await fetch(
    ...
    if (price > 0) cacheSet(ETH_PRICE_CACHE_KEY, price);
    return price;
  } catch {
    return 0;
  }
...
/**
 * Fetch USD prices for ERC-20 tokens (Ethereum mainnet).
 * Stablecoins resolved from hardcodes; others via CoinGecko batch API.
...
export async function fetchTokenPrices(addresses: string[]): Promise<Record<string, number>> {
  ...
  const cached = cacheGet<Record<string, number>>(cacheKey, PRICE_TTL);
