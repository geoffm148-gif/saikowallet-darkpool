...
export function DarkPoolDepositScreen(): React.ReactElement {
  ...
  const { sessionMnemonic, addToast, walletAddress, activeNetworkId, activeAccountIndex } = useContext(AppCtx);
  ...
  async function handleConfirm(): Promise<void> {
    ...
    try {
      ...
      const [nonceHex, gasPriceHex, ethBalHex] = await Promise.all([
        rpcCall<string>(rpcUrl, 'eth_getTransactionCount', [walletAddr, 'pending']),
      ...
      // 2. Deposit (V3: commitment, amount, claimKeyHash)
      setDepositStatus('Sending deposit...');
      const secretHex = '0x' + Array.from(secret).map(b => b.toString(16).padStart(2, '0')).join('');
      const claimKeyHash = ethers.keccak256(ethers.solidityPacked(['bytes32'], [secretHex]));
      ...
      const depositTx = ethers.Transaction.from({
        ...
        data: depositIface.encodeFunctionData('deposit', [commitment, tierAmountWei, claimKeyHash]),
      ...
      try {
        await saveNote(note, walletAddress);
      } catch { /* non-critical */ }
