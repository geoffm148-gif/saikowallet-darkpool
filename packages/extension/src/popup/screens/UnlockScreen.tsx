...
export function UnlockScreen(): React.ReactElement {
  ...
  const handleUnlock = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (resp?.error === 'NO_WALLET') {
        setStorageError('No wallet found. Create a new wallet or recover with your seed phrase.');
        return;
      }
      if (resp?.error === 'CORRUPTED_KEYSTORE') {
        setStorageError('Wallet data is corrupted and cannot be read. Recover with your seed phrase.');
        return;
      }
      ...
      if (!resp?.ok || !resp.mnemonic) {
        setStorageError(resp?.error ?? 'Unknown error occurred. Try recovering with your seed phrase.');
        return;
      }
      ...
      setStorageError('Failed to communicate with the extension. Please restart the browser.');
    } finally {
      setIsLoading(false);
    }
  }, [isLockedOut, passphrase, failedAttempts, setLocked, setSessionMnemonic, setWalletAddress, navigate, addToast, shakeControls]);
        ...
        {storageError && (
          <div style={{
            ...
            <button
              ...
              style={{
                marginTop: SPACING[2], background: 'none', border: `1px solid ${COLORS.error}40`,
                borderRadius: RADIUS.sm, color: COLORS.error, cursor: 'pointer',
                fontFamily: FONT_FAMILY.sans, fontSize: FONT_SIZE.xs, padding: `${SPACING[1]} ${SPACING[2]}`,
                display: 'flex', alignItems: 'center', gap: SPACING[1],
              }}
            >
              <IconKey size={12} /> Recover with Seed Phrase
            </button>
          </div>
        )}
