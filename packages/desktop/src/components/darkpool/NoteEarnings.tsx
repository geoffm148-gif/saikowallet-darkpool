...
export interface NoteEarningsProps {
  ...
  onClaimAll?: () => void;
  onRecover?: (commitment: string) => void;
}

export function NoteEarnings({ notes, totalEarnedSaiko, totalEarnedEth, isLoading, onClaim, onClaimAll, onRecover }: NoteEarningsProps): React.ReactElement {
  const now = Date.now();
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: SPACING[3], flexShrink: 0 }}>
              ...
              {note.isSpent ? (
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => onRecover?.(note.commitment)}
                >
                  Recover
                </Button>
              ) : (
