...
export function DashboardScreen(): React.ReactElement {
  ...
  const loadBalances = useCallback(async () => {
    ...
    try {
      ...
      const [ethRaw, saikoRaw, priceData, ...customResults] = await Promise.allSettled([
      ...
      for (let i = 0; i < customTokenList.length; i++) {
        ...
        if (result && result.status === 'fulfilled') {
          customTokens.push({ token: customTokenList[i]!, balance: result.value as bigint });
        }
      }
      ...
      setBalances((prev) => ({
        ...prev,
        isLoading: false,
        error: 'Network error fetching balances.',
      }));
    }
  }, [address]);
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <span style={{
              ...
              {balances.priceData ? `Updated ${Math.max(1, Math.round((Date.now() - balances.priceData.updatedAt) / 1000))}s ago` : ''}
            </span>
          </div>
        </div>
          ...
          <Card title="Assets" headerAction={
            ...
            {balances.isLoading ? (
              ...
              <div>
                ...
                {/* Add Token link */}
                <div style={{ paddingTop: SPACING[3], textAlign: 'center' }}>
                  <button
