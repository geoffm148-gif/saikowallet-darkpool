...
import { loadCustomTokens } from '../utils/tokens.js';
...
function buildTokenList(): TokenOption[] {
  const custom = loadCustomTokens().map((ct) => ({
    symbol: ct.symbol,
    name: ct.name,
  ...
  const baseAddrs = new Set(
    BASE_TOKENS.filter((t) => t.contractAddress).map((t) => t.contractAddress!.toLowerCase()),
  );
  const unique = custom.filter(
    (t) => t.contractAddress && !baseAddrs.has(t.contractAddress.toLowerCase()),
  );
  return [...BASE_TOKENS, ...unique];
}
...
export function SendScreen(): React.ReactElement {
  ...
  const [tokens, setTokens] = useState<TokenOption[]>(BASE_TOKENS);
  ...
  // Build token list and fetch balances on mount
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const newBalances: Record<string, string> = { ETH: ethBal, SAIKO: saikoBal };

        // Fetch balances for custom tokens
        const customTokens = allTokens.filter(
          (t) => t.contractAddress && t.symbol !== 'SAIKO' && t.symbol !== 'ETH',
        );
        await Promise.all(
          customTokens.map(async (t) => {
            try {
              ...
              const hex = await client.send<string>({
                method: 'eth_call',
                params: [{ to: t.contractAddress, data }, 'latest'],
              });
              const raw = decodeUint256(hex);
              newBalances[t.symbol] = formatBigIntAmount(raw, t.decimals);
            } catch {
      ...
      <div style={CONTENT_STYLE}>
        ...
        <Card bordered padding="lg">
          <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING[5] }}>
            ...
            <TokenSelector selected={selectedToken} onSelect={setSelectedToken} balances={balances} tokens={tokens} />
              ...
              rightAdornment={
                <button
                  ...
                  onClick={() => {
                    ...
                    if (bal && bal !== '—') {
                      const cleanBal = bal.replace(/,/g, '');
                      if (selectedToken === 'ETH') {
                        // Reserve gas: 21000 * 50 gwei fallback
                        const balWei = parseAmountToBigInt(cleanBal, 18);
                        const gasReserve = 21000n * 50_000_000_000n;
                        const adjusted = balWei > gasReserve ? balWei - gasReserve : 0n;
                        setAmount(formatBigIntAmount(adjusted, 18));
                      } else {
                        setAmount(cleanBal);
                      }
                    }
