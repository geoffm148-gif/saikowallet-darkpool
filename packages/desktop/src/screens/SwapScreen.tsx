...
import { fetchTokenMetadata, loadCustomTokens } from '../utils/tokens.js';
...
function TokenSelectModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={title} width={420}>
      ...
      {/* Your Tokens quick-pick (shown when not searching) */}
      {!search && (() => {
        const customTokens = loadCustomTokens();
        const yourTokens: SwapToken[] = [];
        // Always include ETH and SAIKO from the static list
        const eth = tokens.find((t) => t.symbol === 'ETH');
        const saiko = tokens.find((t) => t.symbol === 'SAIKO');
        if (eth && eth.symbol !== excludeSymbol) yourTokens.push(eth);
        if (saiko && saiko.symbol !== excludeSymbol) yourTokens.push(saiko);
        // Add custom tokens not already in the static list
        const staticAddrs = new Set(tokens.map((t) => t.address.toLowerCase()));
        for (const ct of customTokens) {
          if (!staticAddrs.has(ct.address.toLowerCase()) && ct.symbol !== excludeSymbol) {
            yourTokens.push({
              address: ct.address,
              symbol: ct.symbol,
              name: ct.name,
        ...
        if (yourTokens.length === 0) return null;
        return (
                ...
                return (
                  <motion.button
                    ...
                    style={{
                      ...
                      backgroundColor: COLORS.surface,
                      border: `1px solid ${token.featured ? 'rgba(227,27,35,0.3)' : COLORS.border}`,
                      borderRadius: RADIUS.md,
                      cursor: 'pointer',
                      outline: 'none',
                    }}
                    whileHover={{ backgroundColor: COLORS.hoverOverlay, borderColor: COLORS.primary }}
                    whileTap={{ scale: 0.97 }}
                  >
...
function ReviewModal({
  ...
  const isVeryHighImpact = priceImpact > 15;
  const [impactAcknowledged, setImpactAcknowledged] = React.useState(false);
  ...
  return (
    <Modal
      ...
      footer={
        <div style={{ width: '100%' }}>
          {/* Acknowledgement required for very high impact */}
          {isVeryHighImpact && (
            <label style={{
              ...
              <input
                ...
                style={{ marginTop: 2, accentColor: COLORS.error, flexShrink: 0, width: 16, height: 16 }}
              />
              <span style={{
                fontFamily: FONT_FAMILY.sans, fontSize: FONT_SIZE.xs, color: COLORS.textSecondary, lineHeight: 1.5,
              }}>
                I understand this swap has a <strong style={{ color: COLORS.error }}>{priceImpact.toFixed(1)}% price impact</strong> and I may lose significant value.
              </span>
            </label>
          )}
          <Button
            ...
            onClick={onConfirm}
            disabled={isVeryHighImpact && !impactAcknowledged}
          >
      ...
      <div>
        ...
        <div style={rowStyle}>
          ...
          <span style={{
            ...
            fontWeight: isHighImpact ? FONT_WEIGHT.bold : undefined,
          }}>
        ...
        {isHighImpact && (
          <div style={{
            ...
            padding: SPACING[3],
            borderRadius: RADIUS.md,
            backgroundColor: isVeryHighImpact ? `${COLORS.error}18` : `${COLORS.warning}18`,
            border: `1px solid ${isVeryHighImpact ? COLORS.error : COLORS.warning}44`,
            display: 'flex', gap: SPACING[2], alignItems: 'flex-start',
          }}>
            <span style={{ fontSize: 16, flexShrink: 0 }}>{isVeryHighImpact ? '🚨' : '⚠️'}</span>
            <div>
              <div style={{
                fontFamily: FONT_FAMILY.sans, fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold,
                color: isVeryHighImpact ? COLORS.error : COLORS.warning, marginBottom: SPACING[1],
              }}>
                {isVeryHighImpact ? 'Very high price impact' : 'High price impact'}
              </div>
              <div style={{
                fontFamily: FONT_FAMILY.sans, fontSize: FONT_SIZE.xs, color: COLORS.textMuted, lineHeight: 1.5,
              }}>
                {isVeryHighImpact
                  ? `This swap has a ${priceImpact.toFixed(1)}% price impact. You may lose a significant portion of your funds. Consider splitting into smaller trades.`
                  : `This swap has a ${priceImpact.toFixed(1)}% price impact. You will receive less than the market rate.`}
              </div>
            </div>
          </div>
        )}
...
export function SwapScreen(): React.ReactElement {
  ...
  const handleConfirmSwap = async (): Promise<void> => {
    ...
    try {
      ...
      setIsConfirming(false);
      // Log full error to DevTools console for debugging
