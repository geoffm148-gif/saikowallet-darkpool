...
export function SwapScreen(): React.ReactElement {
  ...
  const handleConfirmSwap = async (): Promise<void> => {
    ...
    try {
      ...
      // DIAGNOSTIC: test both IPC bridge and direct fetch
      const w = window as any;
      const hasRpc = !!(w.electronAPI?.rpc);
      let diagMsg = `IPC:${hasRpc ? 'YES' : 'NO'}`;
      if (hasRpc) {
        try {
          const r = await w.electronAPI.rpc.call('https://ethereum.publicnode.com', 'eth_blockNumber', []);
          diagMsg += `|IPC-ok:${r?.result}`;
        } catch(e: any) { diagMsg += `|IPC-err:${e?.message}`; }
      } else {
        try {
          const res = await fetch('https://ethereum.publicnode.com', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'eth_blockNumber',params:[]})});
          const j = await res.json();
          diagMsg += `|fetch-ok:${j?.result}`;
        } catch(e: any) { diagMsg += `|fetch-err:${e?.message}`; }
      }
      addToast({ type: 'success', title: 'Diag', message: diagMsg });
