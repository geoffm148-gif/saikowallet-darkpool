...
import { validateMnemonic, deriveAccount, encryptPayload } from '@saiko-wallet/wallet-core';
...
export function ImportWalletScreen(): React.ReactElement {
  ...
  const handleSetPassphrase = useCallback(async (): Promise<void> => {
    ...
    // Encrypt and persist the seed phrase (same flow as OnboardingScreen)
    if (tab === 'seed' && seedInput.trim()) {
      setIsEncrypting(true);
      try {
        const mnemonicStr = seedInput.trim().split(/\s+/).filter(Boolean).join(' ');
        const keystore = await encryptPayload(mnemonicStr, passphrase);
    ...
    setStep('success');
  }, [passphrase, confirmPassphrase, tab, seedInput, addToast, setSessionMnemonic]);
      ...
      <div style={SCREEN_STYLE}>
        <Card style={CARD_STYLE} bordered padding="lg">
          ...
          <div style={{ display: 'flex', gap: SPACING[3] }}>
            ...
            >
              {isEncrypting ? 'Securing wallet...' : 'Import Wallet'}
            </Button>
          </div>
        </Card>
      </div>
