...
export function DarkPoolScreen(): React.ReactElement {
      ...
      try {
        ...
        await Promise.all(
          loadedNotes.filter(n => !n.isSpent).map(async (note) => {
            try {
              const [earned, earnedEth] = await Promise.all([
                staking.earned!(note.commitment) as Promise<bigint>,
                staking.earnedEth!(note.commitment) as Promise<bigint>,
              ]);
              earnings.set(note.commitment, earned);
              ethEarnings.set(note.commitment, earnedEth);
            } catch {
  ...
  // Claim rewards for a single note
  const handleClaim = useCallback(async (commitment: string) => {
    ...
    try {
      ...
      const staking = new ethers.Contract(DARK_POOL_STAKING_ADDRESS, STAKING_ABI, signer);
      const tx = await staking.claimManual!(commitment, walletAddress);
      await tx.wait();
      addToast({ type: 'success', message: 'Rewards claimed successfully!' });
      const cancelled = { value: false };
      void fetchData(cancelled);
    } catch (e) {
