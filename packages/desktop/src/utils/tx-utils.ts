...
export async function getGasParams(): Promise<{
  ...
  const desiredTip = 1_500_000_000n; // 1.5 gwei preferred
  try {
    ...
    if (block?.baseFeePerGas) {
      ...
      // maxFee = 2x baseFee + tip — always >= tip, handles base fee spikes
      return { maxFeePerGas: base * 2n + desiredTip, maxPriorityFeePerGas: desiredTip };
    }
  } catch { /* fall through */ }
  try {
    const gpRaw = await rpcCall<string>('eth_gasPrice', []);
    const gp = BigInt(gpRaw);
    // Clamp tip so it never exceeds gasPrice (handles ultra-low gas environments)
    const tip = desiredTip < gp ? desiredTip : gp;
    return { maxFeePerGas: gp + tip, maxPriorityFeePerGas: tip };
  } catch { /* fall through */ }
  return { maxFeePerGas: 30_000_000_000n, maxPriorityFeePerGas: desiredTip };
}
...
export async function getNonce(address: string): Promise<number> {
  const raw = await rpcCall<string>('eth_getTransactionCount', [address, 'pending']);
