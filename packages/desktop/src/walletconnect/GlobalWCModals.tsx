...
function RequestModal({ request }: { request: WCRequest }): React.ReactElement {
  ...
  if (request.method === 'personal_sign' || request.method === 'eth_sign') {
    ...
    let valueDisplay = rawValue ?? '0x0';
    try {
      // Convert hex wei to human-readable ETH
      const weiValue = BigInt(rawValue ?? '0x0');
      const ethValue = Number(weiValue) / 1e18;
      valueDisplay = `${ethValue.toFixed(6)} ETH (${rawValue})`;
    } catch { /* leave as hex */ }
    detail = JSON.stringify({ to: tx['to'], value: valueDisplay, data: tx['data'] }, null, 2);
  }
